...
export default function InvestorsPage() {
  ...
  const marketProblems = [
    { icon: CalendarClock, stat: '40%', title: language === 'ar' ? 'تأخر متكرر' : 'Retards fréquents', text: language === 'ar' ? 'الطلبة يفقدون وقتاً يومياً بسبب النقل غير المنظم.' : 'Les étudiants perdent du temps chaque jour à cause d’un transport peu organisé.' },
    ...
    { icon: Network, stat: '0', title: language === 'ar' ? 'حل رقمي موحد' : 'Solution digitale unifiée', text: language === 'ar' ? 'السوق يحتاج منصة واحدة تربط الحجز والتتبع والدفع.' : 'Le marché a besoin d’une plateforme unique reliant réservation, suivi et paiement.' },
  ];
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
                ...
                <div className="mt-3 space-y-2 text-sm font-bold text-slate-600">
                  ...
                  <p>{language === 'ar' ? 'إشراف: الدكتورة رمدوم نورة' : 'Supervision : Dr. RMDOM Noura'}</p>
                </div>
              </div>
            </div>
          </div>
